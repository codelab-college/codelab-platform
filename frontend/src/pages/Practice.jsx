      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-1">Sharpen your skills with practice problems</p>
          </div>
        </div>
            ...
            <div className="bg-[#282828] border border-[#3e3e3e] rounded-xl p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white mt-1">{stats.total_submissions || 0}</p>
                </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-[#1a1a1a] border border-[#3e3e3e] rounded-lg text-white placeholder-gray-500 focus:border-[#ffa116] focus:outline-none"
              />
            </div>
            ...
            <select
              ...
              className="px-4 py-2.5 bg-[#1a1a1a] border border-[#3e3e3e] rounded-lg text-white focus:border-[#ffa116] focus:outline-none"
            >
            ...
            <table className="w-full">
              <thead className="bg-[#1a1a1a]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-400 uppercase">Difficulty</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-400 uppercase">Acceptance</th>
                ...
                {filteredProblems.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="px-4 py-12 text-center">
                      ...
                      <p className="text-gray-400">No practice problems found</p>
                    </td>
                  </tr>
                ) : (
                  filteredProblems.map((problem) => (
                    <tr key={problem.id} className="hover:bg-[#3e3e3e]/30 transition-colors">
                      <td className="px-4 py-4">
                        {problem.is_solved ? (
                          ...
                          <div className="w-5 h-5 rounded-full border-2 border-[#3e3e3e]" />
                        )}
                      </td>
                      <td className="px-4 py-4">
                        <Link to={`/practice/${problem.id}`} className="text-white hover:text-[#ffa116] font-medium transition-colors">
                          {problem.title}
                        </Link>
                        {problem.tags && (
                          <div className="flex gap-1 mt-1">
                            {problem.tags.split(',').slice(0, 3).map(tag => (
                              <span key={tag} className="px-2 py-0.5 bg-[#3e3e3e] text-gray-400 rounded text-xs">
                                {tag.trim()}
                              </span>
                            ))}
                          </div>
